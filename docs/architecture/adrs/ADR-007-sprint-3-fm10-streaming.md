...
- `streaming.py` is committed in Phase 3.C **together with its full unit-test suite** (`tests/test_streaming.py`, covering the module to ~100%) and `tests/test_fm10_regression.py`. Global coverage stays ≥95% on every push — the `cov-fail-under=95` gate (active since 2.F, enforced on every `push: claude/**`) is therefore satisfiable at all times. This mirrors the real Sprint 2 precedent: commit `1d5f18f` (2.C.2) shipped `transitions.py` at 100% module coverage in the skeleton commit itself. **A module is never committed uncovered.**

- The Popperian artifact is the RED→GREEN transition of the *regression test specifically*, not the coverage of the module. In the as-built sequence the regression was observed RED locally at SHA `1049649` (where `tick_streaming` lacked the `advance_cursor` call) and GREEN at `64c3f5e` (coupling added). Those SHAs are preserved in branch history; the local RED observation is reported verbatim by the executor (I1 — report what was measured, not what is convenient). Because the module + tests now land together, the regression test is **non-vacuous by construction**: it is corroborated by two independent assertions —
  (a) behavioural: after a successful tick, `state.json["cursor"]` equals `result.advanced_cursor_to` on disk;
  (b) structural: a spy/mock confirms `tick_streaming` actually *invokes* `advance_cursor` (the test fails if the call is removed, regardless of whether a RED commit exists in public history).
  Either assertion failing refutes the FM-10-closed claim on every future push.
...
**Correction note (L-orq-14).** The original D7 implied `streaming.py` could be committed uncovered at SHA X-1 to "prove" the RED. That was a defect: the coverage gate is global and enforces on every push, so no intermediate uncovered commit can pass CI. Module coverage and the regression RED→GREEN are *orthogonal*: a module can be fully covered while a specific regression assertion is RED in the same commit. Never couple "proving RED" to "leaving the module uncovered". Mandates that introduce a new module must ship skeleton + unit tests in the same commit (cov ≥95% of the file); a regression test may be RED for the target property without dropping coverage.
...
- **DA-2 (replay safety on partial WAL): deferred to Sprint 5 `reconcile`.** `wal.read_events` does NOT filter malformed lines (it calls `json.loads` directly and raises on a corrupt line — verified against `src/atomic_dag/wal.py`). The idempotency replay (D3) therefore assumes a well-formed WAL. Recovery from a partial last line left by a SIGKILL is the domain of the Sprint 5 `reconcile` command, not Sprint 3. (This corrects an earlier draft of DA-2 that wrongly assumed `read_events` skips malformed lines.)
